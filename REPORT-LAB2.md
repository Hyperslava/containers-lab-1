...
Deployment — более высокоуровневый объект, который управляет ReplicaSet и обеспечивает
стратегии обновления, отката и масштабирования.
2. Для чего нужен Service типа ClusterIP? 
Для того, чтобы сервис был доступен только внутри кластера.
3. Как ReplicaSet обеспечивает самовосстановление? 
При удалении какого-то пода из ReplicaSet взамен него создаётся новый.
4. Что произойдет с приложением, если удалить под PostgreSQL? 
Если удалить под PostgresSQL, то приложение продолжит работать, на странице в информации о поде в поле Status значение поменяется на unhealthy, пропадут записи о пользователях из БД. Затем через небольшой промежуток времени автоматически создадится новый под PostgreSQL, в поле Status снова будет записано healthy, и записи о пользователях снова появятся, они подтянутся из volume.
### 7. Выводы 
